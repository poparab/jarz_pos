...
def get_available_delivery_slots(pos_profile_name: str) -> list[dict[str, Any]]:
    ...
    print("🔗 METHOD: get_available_delivery_slots")
...
""",
    ...
    try:
        ...
        try:
            ...
            print("✅ Successfully queried with same_day field")
        except Exception as e:
            print(f"⚠️  same_day field not available, falling back to basic fields: {e!s}")
        ...
        if not day_timings:
            print("🔍 No day timings found, checking all day timings...")
            try:
        ...
        error_msg = f"Error generating delivery slots: {e!s}"
        ...
        traceback.print_exc()
        print("="*80)
        frappe.throw(f"Error loading delivery slots: {e!s}")


def _generate_day_slots(
    target_date: datetime.date,
    opening_time: time | timedelta,
    closing_time: time | timedelta,
    same_day: str,
    slot_hours: int,
    ...
    if current_datetime:
        ...
        if current_slot_time < min_slot_time:
            ...
            if current_slot_time >= end_time:
                print("⚠️  No valid slots for today after time adjustment")
                return slots
    ...
    print("🔄 Starting slot generation loop...")
